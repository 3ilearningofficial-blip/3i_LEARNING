...
function isMultiSessionRole(role: string | undefined | null): boolean {
  const r = String(role || "");
...
/** Whether this token is still valid for this user (primary or multi-session row). */
export async function userHasSessionToken(
  ...
  if (!isMultiSessionRole(row.role as string)) return false;
  const minCreatedAt = Date.now() - ADMIN_SESSION_MAX_AGE_MS;
...
async function persistStaffLoginSession(
  db: DbLike,
  ...
  const platformFamily =
    opts.req ? getActiveSessionPlatformFamily(opts.req) || "web" : "web";
...
export async function persistLoginSession(
  ...
  if (isStaff) {
    await persistStaffLoginSession(db, user, token, deviceId, opts);
    return;
  }
