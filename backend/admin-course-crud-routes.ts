  ...
  app.put("/api/admin/courses/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      await db.query(
        ...
        [title, description, teacherName, price, originalPrice, category, isFree, level, durationHours, isPublished, totalTests, subject, courseType, startDate, endDate, vm, thumbnail ?? null, coverColor ?? null, teacherBio ?? null, teacherImageUrl ?? null, teacherDetailsJson !== undefined ? JSON.stringify(teacherDetails) : null, multiSubjectConfig !== undefined ? JSON.stringify(subjects) : null, courseLanguage ?? null, batchStatus ?? null, req.params.id]
      );
