...
async function resolveStandaloneFolderFullName(db: DbClient, folderId: unknown): Promise<string | null> {
...
async function createStandaloneFolderPath(
  db: DbClient,
  type: string,
  rawName: string,
  rawParentId: unknown,
  ...
  for (let index = 0; index < names.length; index++) {
    ...
    if (existing.rows.length > 0) {
      ...
      if (current.is_hidden) {
        const revived = await db.query("UPDATE standalone_folders SET is_hidden = FALSE WHERE id = $1 RETURNING *", [current.id]);
        current = revived.rows[0];
      }
    } else if (type === "test" && index === names.length - 1) {
      const vm =
        extras.validityMonths != null && String(extras.validityMonths).trim() !== ""
          ? Math.max(0, parseFloat(String(extras.validityMonths)) || 0) || null
          : null;
...
export function registerStandaloneFolderRoutes({
  ...
  app.put("/api/admin/standalone-folders/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (name !== undefined) {
        ...
        const oldFullName = await resolveStandaloneFolderFullName(db, req.params.id);
        ...
        await db.query("UPDATE standalone_folders SET name = $1 WHERE id = $2", [normalizedName, req.params.id]);
        const newFullName = await resolveStandaloneFolderFullName(db, req.params.id);
        if (newFullName) {
          await db.query(
            `WITH target AS (
  ...
  app.delete("/api/admin/standalone-folders/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const fullName = await resolveStandaloneFolderFullName(db, req.params.id);
